...
export default function LevelTwo({ setScene }: Props) {
  ...
  const [coverPresent, setCoverPresent] = useState("present");

  useEffect(() => {
    const removeCover = setTimeout(() => {
      setCoverPresent("notPresent");
    }, 5000);
  }, []);

  function handleWin() {
    setWon(true);
    const moveScene = setTimeout(() => {
      setScene(Scene.Three);
    }, 8000);
  }
  ...
  return (
    <>
      ...
      <div className={`cover ${coverPresent}`}></div>
    </>
  );
}
